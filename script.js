...
(() => {
    ...
    // Visitor Tracking System for Dashboard (Unique Visitors Only)
    async function trackVisitor() {
        try {
            const VISITORS_KEY = 'siteVisitors';
            const VISITOR_ID_KEY = 'uniqueVisitorId';
            const ACTIVITY_LOG_KEY = 'activityLog';

            let visitorId = localStorage.getItem(VISITOR_ID_KEY);

            // If visitor doesn't have an ID, this is their first visit
            if (!visitorId) {
                ...
                // Add to localStorage (backup + offline support)
                ...
                // Add to Firebase (for dashboard sync)
                if (window.db) {
                    try {
                        await db.collection('visitors').add(visitorData);
                        console.log('✅ Visitor tracked in Firebase');
                    } catch (fbError) {
                        console.warn('Firebase visitor tracking failed (will use localStorage):', fbError);
                    }
                }

                // Log activity for new visitors only
            ...
            // If visitor has an ID, they've visited before - don't count them again
        } catch (error) {
            console.error('Visitor tracking error:', error);
        }
    }

    // Track visitor on page load (wait for Firebase to be ready)
